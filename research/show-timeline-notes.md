...
4. **Kick-synced effects live on the grid.** Flames, light hits, CO2 jets, sparkulars and strobe runs are
   `repeat` cues (`every: beat|bar|2bar`, step `pattern`, per-step `cycle` of flame patterns / heights /
...
5. **Blackouts are real blackouts.** 157.9, 232.8–237.4, 409.3–412.3, 531.6–534.7, 612.3–638.6, 787.3–790.3,
   ...
   is kept: in the Discorecord gap the pillar shafts stay red (f016).
...
* All `target` values are `AnchorName`s from `src/core/Anchors.ts` or the filters `all/left/right/center`
  (validator-enforced). `deck_front` is the flame line (the stage module registers it along the whole U: arms +
  side fronts + deck), `towers_top` the castle towers, `roof` the roofline gerb/comet positions, `pillars_top`
  the 8 lantern capitals, `fireworks_sides` the outer/bank mortar lines, `speaker_hangs` the hang tops.
* Where the design bible names a more specific position than the contract has, the cue carries it as
  `p.at` (extension) and a contract `target` that approximates it, so every system can render the cue today:
  | `p.at` | Bible position | Contract fallback used |
  |---|---|---|
  | `flare_pots` | Bengal pots (±86, 9.6, −4.5) | `wing_tips` (2 points) |
  | `corner_towers` | fireballs (±92, 15.5, −4) | `wing_tips` |
  | `tower_torches` | 15 m Power Flames (±14, 16.5, −13) | `speaker_hangs` (hang tops at ±11/±31, Y ≈ 16) |
  | `wing_spars` | 12 spar flame heads | `wing_left` / `wing_right` |
  | `arms` | arm posts (±93, 5.9, Z 2…58) | `deck_front` |
  | `hang_lines` | glitter streams (±11, 16.5, −4) | `speaker_hangs` |
  Pillar-row selections use `p.rows` (1 = nearest the stage); systems that ignore it fire all 8 capitals.
...
| 55.30–69.00 | Bengal flares (hits 2–3) | orchestral · 0.55 · `winter_fire` | burst(bengal) flare_pots→wing_tips; flame deck_front | f006 | FACT red sources at the front-line corners + red field (drone) · INFERENCE Bengal |
...
| 97.00–110.00 | second half, corner fireballs | orchestral · 0.6 · `winter_night` | flame corner_towers→wing_tips | f010 | FACT two orange fireballs at the outer structures · INFERENCE 100.1 hit · ASSUMPTION echo at 103.2 |
...
| 219.00–231.23 | build, comet fan from the crest | build · 0.6 · `disco_crown` | comet 7 gold 38 m; comet 12 #F0F0FF 40 m | f022–f023 | FACT gold V-fan from the crest, silver glitter fan behind the dragon |
| 231.23–243.46 | pre-drop | build · 0.6 · `disco_dark` | — | f024 | FACT gap 232.8–237.4 + cool-white beam fans · ASSUMPTION strobe roll |
| 243.46–267.92 | DROP | drop · 0.9 · `disco_drop` | laser cone cyan @deck_front; gerb wing_tips; comet 18 white 30 m; comet 20 #FFE8B0 35 m; laser sky cyan @roof; comet 16 #FFE8B0 32 m; comet 14 white 28 m | f025–f026 | FACT comet V-fans + white comet row + fountain walls in green smoke · ASSUMPTION kick lighting |
...
| 330.28–341.12 | kick 2, comet columns | drop · 0.7 · `oath_drop` | gerb deck_front; comet 20 #FF3060 40 m | f033–f034 | FACT vertical comet/gerb columns across the full width, striped wings, white outer crystals |
...
| 372.08–403.05 | the Oath: blue sunburst & red stars | vocal · 0.35 · `oath_vocal` | laser fan #3040FF @deck_front; comet 14 red 50 m | f038–f040 | FACT blue sunburst laser fans + dashed ground lines (375), red comet row across X ±95 (385) |
...
| 464.99–477.38 | quiet, low fog | breakdown · 0.3 · `oath_quiet` | laser sweep white @deck_front; jet center | f047–f048 | FACT hero cam: jagged white lines, near-blackout deep blue, low horizontal lavender fans, CO2 plume, dense low fog |
...
| 502.15–508.34 | DROP 2 (violet starbursts) | drop · 0.9 · `oath_violet2` | laser fan white @roof; flame deck_front; flame deck_front ×x-x-/beat | f051 | FACT violet look, white starburst rosettes, window LED bars, mouth glow, warm crystals, purple pillar bodies |
| 508.34–523.83 | DROP 2 (blue & red) | drop · 0.9 · `oath_blue` | laser crossfire white @fireworks_sides; flame deck_front ×x---x-x-/beat; flame deck_front ×x-x-x-xx/beat | f052–f053 | FACT deep blue except red-orange dragon + wings, white V-fans from the side sections (523), blue pillar bodies |
...
| 536.21–564.09 | FINAL KICK, the red canopy | climax · 0.95 · `oath_red` | laser fan #A040FF @fireworks_sides; flame deck_front; gerb roof; salvo crackle 8 red 62 m; cake crackle 36 shots red; flame deck_front ×x-x-x-x-x-x-xxxx/beat; flame deck_front ×x---/beat | f054–f056 + photo P | FACT red look, white roofline gerbs, red/white crackle canopy 45–80 m in two lobes, gold brocade line (553) · ASSUMPTION flames |
...
| 709.25–718.25 | BURNING WINGS | drop · 0.8 · `flame_fire` | firewall wing_spars→wing_left+wing_right | f072 | FACT both wings engulfed, everything orange/amber, yellow rosettes · INFERENCE 709.25 downbeat · ASSUMPTION echo bursts 715.25/718.25 |
...
| 745.25–769.25 | silver glitter comets | drop · 0.6 · `flame_glitter` | cake 10 shots #FFF0E0; comet 1 white 50 m; comet 4 #F0F0FF 50 m; comet 2 #F0F0FF 60 m | f076–f077 | FACT white lamp row, orange-red centre, cyan-white side sections, silver glitter streams from the hang lines (760) |
| 769.25–781.25 | the glitter curtain | drop · 0.65 · `flame_glitter` | flame pillars_top; gerb pillars_top; comet 14 #F0F0FF 55 m; salvo willow 8 gold 80 m; salvo strobe 6 white 72 m | f078–f079 | FACT 10+ silver glitter comets to X ±110, gerbs on row-2 capitals, full-sky curtain of white/gold glitter (780) |
...
| 793.25–817.25 | DROP 1 | drop · 0.92 · `flame_climax` | flame deck_front; jet pillars_base; comet 8 red 45 m; flame deck_front ×x/beat; flame center ×x---/beat; flame deck_front ×x-x-x-x-x-x-xxxx/beat; flame wing_spars→wing_left ×x-------/beat | f081–f082 | FACT red haze, near-blackout accent with cool-white crystals 799–806, ≥7 red/white glitter comet columns · ASSUMPTION kick flames |
...
| 829.25–859.25 | MAIN CLIMAX | climax · 1 · `flame_climax` | firewall wing_spars→wing_left+wing_right; flame deck_front; salvo strobe 15 red 40 m; flame deck_front ×x-x-x-x-x-x-xxxx/beat; laser sheet #8A2BFF; cake crackle 16 shots #FF3060; comet 1 orange 50 m | f084–f086 | FACT red in thick haze, red strobe-star arc, crackle + orange comet (839), orange X-fans at the arm ends (849), booth strobe + white down-beams · ASSUMPTION flame choreography |
| 859.25–877.25 | THE FLAME RING | climax · 1 · `flame_ring` | flame deck_front ×x/bar; flame pillars_top ×x/bar; flame tower_torches→speaker_hangs; flame deck_front ×x---/beat; firewall wing_spars→wing_left+wing_right | f087–f088 | FACT ~60 flame heads along deck + sides + arms as one line, flames on all 8 capitals, stage fixtures off (lit by fire only) · INFERENCE 859.25 downbeat |
...
| 1026.90–1046.10 | CLIMAX 2: the twin torches | climax · 0.88 · `domitor_violet` | flame tower_torches→speaker_hangs; flame deck_front ×x---x---x-x-x---/beat | f104–f105 | FACT twin 15 m flame torches flanking the head (f104), magenta dragon, purple pillars, blue/violet wings with pink-orange dashes (1037) · ASSUMPTION kick flames |
| 1046.10–1049.30 | blackout accent | anticlimax · 0.4 · `domitor_blue` | — | f106 | FACT blackout with a faint teal outline (1047) |
| 1049.30–1065.30 | tamed: deep blue & glitter | climax · 0.8 · `domitor_blue` | flame deck_front ×x-------/beat; flame tower_torches→speaker_hangs | f107 | FACT stage deep blue with white/cyan glitter pixels, near pillars green cones, mid pillars purple crystals |
| 1065.30–1074.90 | magenta & ice | climax · 0.8 · `domitor_violet` | flame deck_front ×x---x-x-/beat; comet 6 white 40 m | f108 | FACT magenta/pink-violet + orange, ice-blue crystals, orange shafts, grey dots rising (shells? UNKNOWN) |
| 1074.90–1090.90 | the comet barrage | climax · 0.88 · `domitor_violet` | flame tower_torches→speaker_hangs; flame deck_front ×x-x-x-x-x-x-xxxx/beat; comet 12 #FFF0D8 40 m; comet 10 #FF6030 45 m; comet 12 #FFF0D8 38 m; comet 10 #FF6030 42 m; cake 16 shots #FFF0D8 | f109 | FACT ~12 white-gold comets from the front + arms fanning outward + ~10 orange-red comets in two crossing fans, central smoke plume, cyan balustrade lamps |
...
| 1155.40–1182.40 | kick & lead, the fountain row | drop · 0.72 · `embers_ice` | laser sky #AED3F2 @wing_right; laser sheet #2040FF; gerb deck_front; comet 17 white 30 m | f117–f119 | FACT upward cyan-white fan from the right side (1156), blue sheet at deck height (1165), gold spire outlines, 18–20 white-gold fountains + a row of ~17 comet heads (1175) |
| 1182.40–1188.40 | pre-drop gap | silence · 0.3 · `embers_ice` | — | — | INFERENCE gap (audio minimum 1185.1) · ASSUMPTION blackout + strobe |
| 1188.40–1194.40 | DROP 1: the pink crackle band | drop · 0.92 · `embers_pink` | laser cone cyan @deck_front; gerb arms→deck_front; jet deck_front ×x---x---x---x-x-/beat; salvo crackle 14 #FF60C0 70 m | f120 | FACT field flooded pink by pyro, tall pink-white sprays along both arms, continuous pink/white crackle band 60–80 m, pink smoke in the aisle |
...
| 1273.50–1287.00 | DROP 2: magenta | drop · 0.85 · `embers_magenta` | laser fan #FF40C0 @roof; jet deck_front ×x---x---x---x-x-/beat; salvo crackle 8 #FF40C0 50 m; salvo crackle 8 white 55 m | f129 | FACT magenta/violet stage, cyan-green crystals, magenta-lit smoke column centre-right, pink/magenta + white crackle 1.5–2x stage height |
| 1287.00–1305.00 | FINAL CLIMAX: pink canopy over a cyan stage | climax · 0.95 · `embers_climax` | gerb arms→deck_front; jet deck_front ×x---x---x---x-x-/beat; comet 9 #FFC0E0 30 m; salvo crackle 16 #FF2A50 80 m; salvo kamuro 10 #FFB0C0 90 m; salvo crackle 12 #FF60C0 70 m | f130–f131 | FACT 9 white-pink comet jets from the roof/front bursting into a crackle canopy, ~6 white fountains per arm, HUGE red/pink crackle-glitter canopy ~90 m (1293.4), cyan/teal stage |
...
| 1402.79–1408.99 | build: green mines | build · 0.45 · `cold_red` | comet 6 #FFC0E0 20 m; mine green 12 m | f142 | FACT ~6 small green-white bursts just above the roofline + thin pink/white comet trails (on the grid at 1402.8) |
| 1408.99–1412.08 | silence | silence · 0.2 · `cold_red` | — | — | INFERENCE silence ~1410.5–1412 · ASSUMPTION blackout + roll |
| 1412.08–1436.86 | DROP 1: the silver gerb wall | drop · 0.85 · `cold_drop` | flame deck_front; comet 8 #FFA21A 35 m; flame deck_front ×x---x---x---x-x-/beat; gerb deck_front; cake 16 shots white; comet 8 white 25 m; flame deck_front ×x-x-x-x-x-x-xxxx/beat | f143–f145 | FACT magenta/pink + orange key light, pink LED dot rows, WHITE/SILVER GERB WALL across front + sides + arms (1422.9), white fans at the arm ends, gold comet streaks |
| 1436.86–1452.34 | drop 1 tail | drop · 0.75 · `cold_drop` | flame deck_front ×x-------/beat | f146 | UNKNOWN (aerial shows the stage dark: camera cut or blackout accent) · ASSUMPTION dimmed kick look |
...
| 1498.79–1511.18 | final build-up | build · 0.72 · `cold_red` | mine white 15 m; shell crossette red 35 m; flame deck_front ×x/beat | f152 | FACT intense red/pink structure, white/silver + red low bursts just above the dragon (roof mines / crossettes), red streaks up-right · ASSUMPTION build flames |
| 1511.18–1523.57 | GRAND FINALE | climax · 1 · `finale` | firewall wing_spars→wing_left+wing_right; flame deck_front; flame tower_torches→speaker_hangs; gerb deck_front; gerb roof; gerb pillars_top; finale  70 m | f153 | FACT field glows orange-red, gold gerb/fountain wall along the whole U, gold/orange crackle + brocade band 50–90 m across ±100 m, 2 white-gold chrysanthemums at X ±45 · INFERENCE flame chase per bar |
| 1523.57–1529.76 | finale afterglow | climax · 0.85 · `finale` | salvo willow 10 gold 80 m | f154 | FACT heavy residual smoke, fading white-gold falling stars · ASSUMPTION flame chase continues |
...
| 1535.95–1551.00 | blue cold fire | breakdown · 0.4 · `outro_blue` | shell kamuro gold 80 m; flame deck_back ×x/2bar; jet towers_top ×x/2bar | f156–f157 | FACT white-pink core, cyan arm lights, blue/cyan-lit plumes in 2 groups of 3–4 columns flanking the core (1541), fading gold glitter · INFERENCE blue-lit CO2 (+ blue flame read) |
| 1551.00–1557.00 | the final swell | build · 0.6 · `outro_blue` | flame deck_back; jet towers_top+deck_back | f157 | INFERENCE swell peak 1554.8 · ASSUMPTION last cold-fire blast |
...
flame chase (show-analysis 1.4). ASSUMPTION: flame accents inside the climax at 81.0/84.9/92.9/95.5 (free
tempo, placed on the waveform plateau, alternating chase directions), a fireball echo at 103.2, a pale sky beam
from the dragon's skull fixtures in the 110–118 hush, strobe hits on 114.4/120.3. No lasers (FACT: none in any
Winter frame).
...
silence 409.3–412.3 is a total blackout (the "DEF-QON!" chant moment). ASSUMPTION: anthem-drop flames on the kick
(the storyboard frames of the drops are aerials/close-ups that do not rule them out; flames are the default
Defqon vocabulary for an anthem drop), white CO2 on drop downbeats, strobe rolls before each drop.
...
FLAME RING (f087) and the red wings with dashes (f088). INFERENCE: flame ring on the 859.25 downbeat, re-fired on
4 downbeats (one continuous 6 s ring) then a centre-out chase every bar to 877.25; capitals burn as torches for
the whole 18 s. ASSUMPTION: tom-synced red light pulses in the ritual (the toms carry the groove; no hardstyle
kick), wing-flame echoes at 712.25/715.25/718.25, kick-flame choreography in drop 1 and the climax (patterns
cycle lr → rl → centre-out → out-centre → alternate → all; heights ramp 6 → 12 m over the 4 bars before the
ring), the wings burning again at 874.25 as a bookend.
...
re-fired every 2–4 bars in climax 2, deck flames on the kick from 1033.3.
...
light hits; blackout accents 1212.4–1216.9 carry strobe hits on the dips.
...
blue `flame` plumes on the castle terrace so the image reads as "blue fire" either way. ASSUMPTION: flames on the
kick in drop 1, the flame chase through the finale (big on 1, small on 3), 15 m torches every 2 bars in the
...
| S4 | 69.5–95.5 | gerbs on the lanterns, V-fans | `pyro.gerb pillars_top rows 1–2` + centre; roof/wing gerbs; deck comet V-fans |
| S5 | 206.77–219 | lasers skim the empty field | `lasers.look wave/fan/sky` green-cyan-blue + field low fog |
| S6 | 243.46 | Discorecord drop | white comet row (`roof`) + V-fans (`fireworks_sides`), fountain cakes at bar 8 |
| S7 | 341.12–403 | E-Life on the empty stage | deck cams, cyan-white `audience` backlight curtain from 353.5 |
| S8 | 409.25–415.44 | silence → anthem drop | total blackout, 8→16 Hz strobe roll, 20 red comets in a 150° fan |
| S9 | 536.21–557.9 | the red canopy + gold brocade line | 8 crackle salvos alternating ±45 m lobes + crackle cake; brocade line at 554.8 |
...
| S12 | 709.25 | burning wings | `pyro.firewall wing_left+wing_right` 3 s + echoes; orange world |
| S13 | 776.75–778.25 | glitter sky | willow + strobe salvos 220 m wide at 72–80 m |
| S14 | 859.25 | the flame ring | `flame deck_front` ×4 bars + capital torches + 15 m pair; fixtures dark |
| S15 | 887.2–933.8 | the piano laser | `lasers.look crossfire origin field` with `segments` 1→8, pillar cross-beams |
| S16 | 933.8 / 1026.9 | the dragon wakes / twin torches | `stage.state rage` green/red + `roar`; 15 m torches (`p.at: tower_torches`) |
| S17 | 1074.9 | crossing comet fans | white-gold front fans + orange crossing fans, re-fired at 1078.1 |
...
| S20 | 1287–1297 | pink canopies over a cyan stage | 9 crackle comets, 16-shell red/pink crackle + kamuro canopy |
...
| S24 | 1511.18 | grand finale | gerb wall on the whole U (12.4 s), `fireworks.finale` 18/s + 2 chrysanthemums ×2 |
| S25 | 1540.6–1554.8 | blue cold fire, then black | blue flame plumes + CO2 every 2 bars, last blast 1554.8, black at 1561 |
...
**Sight lines (checked by the generator against boxes).** The research positions A (0, 3, 175) and B
(0, 1.8, 172) stand behind / under the bible's own photo terrace (Z 166–172, deck Y 5), and the world module
builds a FOH/press tower on the axis at X ±8, Z 150–160 (roof 9.6 m) and the lantern pillars at X ±22,
Z 46.5 / 73.5 / 100.7 / 127.7 (14.8 m; `src/world/site.ts` of the world worktree — the bible put them at X ±20,
Z 36–135). Every far camera therefore stands on the tower's front, Z ≤ 147: A at (±6, 8.5, 146) with the row-3
pillar pair at the frame edges, B at (−12, 6.8, 146) (the official photographer's height, pitch ≈ +9°), far C/E
cams at Z 147, the official-photo shot of the red canopy at (0, 11, 147) just above the tower's front. C cameras
are ≥ 6.5 m high (camera risers — above Tribe-mode flags, which are up to 4.5 m) and E cams 2.4 m. The generator
checks each shot's start and end pose twice: the centre sight line against all boxes (bible and world layouts),
and the screen area that a world-build structure standing between the lens and the subject covers (≤ 17 %: only
pillars framing the picture at its edges remain). The K shot looks *at* the L1 lantern (−22, 13, 46.5) on
purpose. On-deck cameras stand 1.2 m above the deck-front fixture row, so every static `still` look of the
`floor` group carries `tilt: 38` (up-lights into the haze; the rig's default 4° points them into the front rows
and into the lens). The auto director only takes over if a shot is removed.
...
* The exact Bengal / corner-tower / torch / arm positions need the extended anchors (`p.at`); until then the
  fallback targets named in §4 are used (visible consequences in the integration preview: the twin torches
  fire from all 4 hang tops, the Bengals burn on the wing tips instead of the front-line corners).
...
* Field layout: the camera positions follow the world module's build (pillars X ±22 at Z 46.5–127.7, FOH tower
  Z 150–160). All effects address the pillars through anchors, so only the explicit camera coordinates depend
  on it; if the world layout moves back to the bible's (X ±20, Z 36–135), re-check the K shot and the side
  telephoto of the Oath/L.P.A. link (the generator's box check covers both layouts for the centre sight line).
